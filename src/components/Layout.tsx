...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div className="w-64 bg-blue-900 text-white shadow-lg transition-all duration-300 ease-in-out">
        ...
        <div className="p-4">
          <div className="flex items-center space-x-3 mb-6 p-3 bg-blue-800 rounded-lg">
            ...
            <ul className="space-y-1">
              {navLinks.map((link) => (
                <li key={link.to}>
                  <Link
                    to={link.to}
                    className={`flex items-center space-x-3 p-3 rounded-lg transition-colors duration-200 hover:bg-blue-800 ${
                      location.pathname === link.to ? 'bg-blue-800 font-medium' : 'text-blue-200'
                    }`}
                  >
                    {link.icon}
              ...
              <li className="pt-6">
                <button
                  ...
                  className="w-full flex items-center space-x-3 p-3 rounded-lg text-blue-200 transition-colors duration-200 hover:bg-red-700 hover:text-white"
                >
                  <LogOut size={20} />
                  <span>Logout</span>
                </button>
